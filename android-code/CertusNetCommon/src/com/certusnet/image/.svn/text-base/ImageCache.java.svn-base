...
public class ImageCache {
	...
	private ImageCache(Context context) {
		...
		if (selfPath) {
			String baseDir = FileUtils.getStorePath(context).getAbsolutePath();
			// String.valueOf(Environment.getExternalStoragePublicDirectory("chenqingmin.test")
			// .getAbsolutePath());
	...
	public Bitmap getBitmapFromDisk(String url, float scale) {
		...
		if (diskCacheEnabled) {
			...
			if (new File(filePath).exists()) {
				BitmapFactory.Options options = BitmapUtil.getScaleBitmapFactoryOption(scale);
	...
	public Bitmap getBitmapFromDisk(String url, float mWidth, float mHeight) {
		...
		if (this.diskCacheEnabled) {
			...
			if (new File(ilePath).exists())
				bitmap = BitmapLoader.getBitmapFromDisk(ilePath, mWidth, mHeight);
		}
		return bitmap;
	}
